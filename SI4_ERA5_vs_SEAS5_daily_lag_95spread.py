# -*- coding: utf-8 -*-
"""
Fig.S4 daily evolution by two periods + ALL-PERIOD (bottom two rows):
...
OUT_NPZ = OUTPUT_DIR / "SI4_ERA5_vs_SEAS5_daily_lag_twoPeriods_95memberSpread_cache_newNA.npz"

# separate cache for the all-period (full record) rows
OUT_NPZ_ALL = OUTPUT_DIR / "SI4_ERA5_vs_SEAS5_daily_lag_ALLPERIOD_95memberSpread_cache_newNA.npz"

OUT_FIG = OUTPUT_DIR / "SI4_ERA5_vs_SEAS5_daily_lag_twoPeriods_95memberSpread_newNA.pdf"
...
#  all-period key/label, covering the entire record (all years)
ALL_PERIOD_KEY = "pall"
...
# every year 1981-2024 maps to the single all-period key
def all_period_key_from_year(year):
    if START_YEAR <= year <= END_YEAR:
        return ALL_PERIOD_KEY
    return None
...
# ================================================================
# BASELINE CLIMATOLOGY (unchanged)
# ================================================================
...
# ================================================================
# SAVE / LOAD CACHE (decade caches, unchanged format)
# ================================================================
def save_cache(npz_path, era5, seas5, era5_counts, seas5_counts, period_keys=None):
    if period_keys is None:
        period_keys = PERIOD_ORDER
...
def load_cache(npz_path, period_keys=None):
    if period_keys is None:
        period_keys = PERIOD_ORDER
    ...
    era5 = {pk: {"raw": {}, "det": {}} for pk in period_keys}
    seas5 = {pk: {"raw": {}, "det": {}} for pk in period_keys}
    era5_counts = {pk: {"raw": {}, "det": {}} for pk in period_keys}
    seas5_counts = {pk: {"raw": {}, "det": {}} for pk in period_keys}
...
# ================================================================
# PLOT (extended to 6 rows: 4 decade rows + 2 all-period rows)
# ================================================================
def plot_fig(era5, seas5, era5_all=None, seas5_all=None):
    print(f"\nPlotting -> {OUT_FIG}")
    PERIOD_COLORS = {
        "p1": "#1C6AB1",       # 1981-1990
        "p2": "#ED4043",       # 2015-2024
        ALL_PERIOD_KEY: "#2E9E4A",  # all-period colour (green)
    }
    ...
    add_all_rows = (era5_all is not None) and (seas5_all is not None)
    ...
    era5_by_row_source = {i: era5 for i in range(4)}
    seas5_by_row_source = {i: seas5 for i in range(4)}


    if add_all_rows:
        row_info = row_info + [
            ("raw", ALL_PERIOD_KEY, f"{ALL_PERIOD_LABEL}\nRaw"),
            ("det", ALL_PERIOD_KEY, f"{ALL_PERIOD_LABEL}\nDetrended"),
        ]
        era5_by_row_source[4] = era5_all
        era5_by_row_source[5] = era5_all
        seas5_by_row_source[4] = seas5_all
        seas5_by_row_source[5] = seas5_all
    ...
    if add_all_rows:
        for tag in ["raw", "det"]:
            for rn in REGION_ORDER:
                member_series = seas5_all[ALL_PERIOD_KEY][tag][rn]
                all_vals.append(np.nanpercentile(member_series, 2.5, axis=0))
                all_vals.append(np.nanpercentile(member_series, 97.5, axis=0))
                all_vals.append(era5_all[ALL_PERIOD_KEY][tag][rn])
    ...
    for r, (tag, pk, row_label) in enumerate(row_info):
        era5_src = era5_by_row_source[r]
        seas5_src = seas5_by_row_source[r]


        for c, rn in enumerate(REGION_ORDER):
            ax = axes[r, c]
            col = PERIOD_COLORS[pk]
            member_series = seas5_src[pk][tag][rn]
            ...
            era5_series = era5_src[pk][tag][rn]
    ...
    if add_all_rows:
        legend_handles.append(
            mlines.Line2D([], [], color="#2E9E4A", lw=2.2, label=f"SEAS5 mean, {ALL_PERIOD_LABEL}")
        )
...
def main():
    print("=" * 80)
    print("Fig.S2 daily evolution: two periods + all-period bottom rows")
    ...
    if OUT_NPZ.exists():
        ...
        era5_ssw = None
        seas5_ssw = None
        era5_clim = seas5_clim = None
        era5_trend = seas5_trend = None
    ...
    #  all-period (full 1981-2024, all SSW events, not split by decade)
    if OUT_NPZ_ALL.exists():
        era5_all, seas5_all, era5_counts_all, seas5_counts_all = load_cache(
            OUT_NPZ_ALL, period_keys=[ALL_PERIOD_KEY]
        )
        ...
        era5_all, era5_counts_all = calc_era5_lag_series_by_period(
            era5_ssw, era5_clim, era5_trend,
            period_keys=[ALL_PERIOD_KEY],
            period_key_func=all_period_key_from_year
        )


        seas5_all, seas5_counts_all = calc_seas5_member_lag_series_by_period(
            seas5_ssw, seas5_clim, seas5_trend,
            period_keys=[ALL_PERIOD_KEY],
            period_key_func=all_period_key_from_year
        )
        ...
        save_cache(
            OUT_NPZ_ALL,
            era5_all, seas5_all, era5_counts_all, seas5_counts_all,
            period_keys=[ALL_PERIOD_KEY]
        )


    plot_fig(era5, seas5, era5_all=era5_all, seas5_all=seas5_all)


    del era5, seas5, era5_counts, seas5_counts
    del era5_all, seas5_all, era5_counts_all, seas5_counts_all
    gc.collect()
