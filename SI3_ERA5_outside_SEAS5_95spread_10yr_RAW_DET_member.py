# -*- coding: utf-8 -*-
"""
Fig.S3 10-year RAW + DETRENDED + ALL-PERIOD (bottom row):
SEAS5 mean post-SSW T2m anomaly
+ hatching where ERA5 composite is outside SEAS5 95% member-event spread.

Layout:
    6 rows x 2 columns
    Rows 1-5: decade panels 
    Row 6   : entire 1981-2024 period, using ALL ERA5/SEAS5 SSW events
                 (not split by decade) -> smallest sampling uncertainty
    Left:  Raw
    Right: Detrended
...
# NEW: separate cache files for the all-period (full record) row
OUT_NPZ_RAW_ALL = OUTPUT_DIR / "SI3_ERA5_outside_SEAS5_95spread_ALLPERIOD_RAW_member.npz"
OUT_NPZ_DET_ALL = OUTPUT_DIR / "SI3_ERA5_outside_SEAS5_95spread_ALLPERIOD_DET_member.npz"
...
# NEW: label for the additional bottom row using the entire record
ALL_PERIOD_LABEL = "1981-2024 (All)"
...
# (unchanged logic; period_filter_func lets us reuse this for the
#  all-period row without touching the decade computation itself)
# ================================================================
def calc_era5_composites_by_period(ssw_df, clim, detrend=False, slopes=None,
                                    period_labels=None, period_filter_func=None):
    if period_labels is None:
        period_labels = [label for _, _, label in DECADE_PERIODS]
    if period_filter_func is None:
        period_filter_func = get_period_label

    mode = "detrended" if detrend else "raw"
    print(f"\nCalculating ERA5 {mode} composites by period...")


    period_samples = {label: [] for label in period_labels}
    lat_vals = lon_vals = None
...
def calc_seas5_samples_by_period(ssw_df, clim, detrend=False, slopes=None,
    ...
    mode = "detrended" if detrend else "raw"
    print(f"\nCalculating SEAS5 {mode} 25 member-mean composites by period...")
    ...
    period_event_counts = {label: 0 for label in period_labels}
...
# ================================================================
# BUILD FINAL MODE RESULT (decade rows) - UNCHANGED
# ================================================================
def build_mode_result(mode_name, out_npz, era5_ssw, seas5_ssw,
    ...
    if out_npz.exists():
        ...
        for _, _, label in DECADE_PERIODS:
            results[label] = {
                ...
                "seas5_n": int(data[f"{label}__seas5_n"]),
            }
    ...
    for _, _, label in DECADE_PERIODS:
        ...
        results[label] = {
            ...
            "era5_n": era5_results[label]["n"],
            "seas5_n": seas5_results[label]["n"],
        }
...
# ================================================================
# NEW: BUILD ALL-PERIOD RESULT (bottom row)
# Uses the entire 1981-2024 record, all ERA5/SEAS5 SSW events,
# NOT split by decade. Reuses the same composite/spread functions.
# ================================================================
def build_all_period_result(mode_name, out_npz, era5_ssw, seas5_ssw,
                             era5_clim, seas5_clim,
                             era5_slopes=None, seas5_slopes=None):
    detrend = mode_name.upper() == "DETRENDED"
    ...
    if out_npz.exists():
        print(f"\nLoading cached ALL-PERIOD {mode_name}: {out_npz}")
        data = np.load(out_npz)
        ...
        return {label: result}, lat_vals, lon_vals

    all_period_filter = lambda year: label
    ...
    era5_results, era5_lat, era5_lon = calc_era5_composites_by_period(
        ...
        slopes=era5_slopes,
        period_labels=[label],
        period_filter_func=all_period_filter
    )
    ...
    save_dict = {
        "lat": lat_vals,
        "lon": lon_vals,
        f"{label}__seas5_mean": result["seas5_mean"],
        f"{label}__seas5_lower": result["seas5_lower"],
        f"{label}__seas5_upper": result["seas5_upper"],
        f"{label}__era5_interp": result["era5_interp"],
        f"{label}__outside_mask": result["outside_mask"],
        f"{label}__era5_n": np.array(result["era5_n"]),
        f"{label}__seas5_n": np.array(result["seas5_n"]),
    }
...
# ================================================================
# PLOT RAW + DETRENDED + ALL-PERIOD (NEW bottom row)
# ================================================================
def plot_raw_det_maps(raw_results, det_results, lat_vals, lon_vals,
                       raw_all_results=None, det_all_results=None):
    print(f"\nPlotting: {OUT_FIG}")
    ...
    # NEW: rows = decade rows + 1 extra row for the all-period comparison
    add_all_row = (raw_all_results is not None) and (det_all_results is not None)
    row_labels = [label for _, _, label in DECADE_PERIODS]
    row_results_raw = [raw_results] * len(DECADE_PERIODS)
    row_results_det = [det_results] * len(DECADE_PERIODS)

    if add_all_row:
        row_labels = row_labels + [ALL_PERIOD_LABEL]
    ...
    for r, label in enumerate(row_labels):
        is_all_row = add_all_row and (r == len(DECADE_PERIODS))

        for c, col_title in enumerate(["Raw T2m", "Detrended T2m"]):

            if is_all_row:
                mode_results = raw_all_results if c == 0 else det_all_results
            else:
                mode_results = raw_results if c == 0 else det_results
            ...
            ax.set_title(
                f"{title_label} (n={seas5_n})",
                fontsize=12, fontweight=title_fontweight, pad=4
            )
...
def main():
    print("=" * 80)
    print("Fig.S1 10-year RAW + DETRENDED + ALL-PERIOD bottom row")
    print("SEAS5 mean + ERA5 outside SEAS5 95% member spread")
    print("=" * 80)


    decade_caches_exist = OUT_NPZ_RAW.exists() and OUT_NPZ_DET.exists()
    all_caches_exist = OUT_NPZ_RAW_ALL.exists() and OUT_NPZ_DET_ALL.exists()


    if decade_caches_exist and all_caches_exist:
        print("\nAll caches (decade + all-period) exist. Skip all calculations.")


        raw_results, lat_vals, lon_vals = build_mode_result(
            mode_name="RAW", out_npz=OUT_NPZ_RAW,
            era5_ssw=None, seas5_ssw=None,
            era5_clim=None, seas5_clim=None,
            era5_slopes=None, seas5_slopes=None
        )

        det_results, _, _ = build_mode_result(
            mode_name="DETRENDED", out_npz=OUT_NPZ_DET,
            era5_ssw=None, seas5_ssw=None,
            era5_clim=None, seas5_clim=None,
            era5_slopes=None, seas5_slopes=None
        )

        raw_all_results, _, _ = build_all_period_result(
            mode_name="RAW", out_npz=OUT_NPZ_RAW_ALL,
            era5_ssw=None, seas5_ssw=None,
            era5_clim=None, seas5_clim=None,
            era5_slopes=None, seas5_slopes=None
        )

        det_all_results, _, _ = build_all_period_result(
            mode_name="DETRENDED", out_npz=OUT_NPZ_DET_ALL,
            era5_ssw=None, seas5_ssw=None,
            era5_clim=None, seas5_clim=None,
            era5_slopes=None, seas5_slopes=None
        )
        ...
        plot_raw_det_maps(
            raw_results, det_results, lat_vals, lon_vals,
            raw_all_results=raw_all_results, det_all_results=det_all_results
        )


        del raw_results, det_results, raw_all_results, det_all_results
        gc.collect()
    ...
    raw_results, lat_vals, lon_vals = build_mode_result(
        mode_name="RAW", out_npz=OUT_NPZ_RAW,
        era5_ssw=era5_ssw, seas5_ssw=seas5_ssw,
        era5_clim=era5_clim, seas5_clim=seas5_clim,
        era5_slopes=None, seas5_slopes=None
    )
    ...
    det_results, _, _ = build_mode_result(
        ...
        era5_ssw=era5_ssw, seas5_ssw=seas5_ssw,
        era5_clim=era5_clim, seas5_clim=seas5_clim,
        era5_slopes=era5_slopes, seas5_slopes=seas5_slopes
    )


    # build the all-period (full 1981-2024, all events) bottom row.
    # Reuses era5_slopes/seas5_slopes computed above if available;
    # otherwise (all decade caches existed already) recompute them
    # only if the all-period detrended cache doesn't exist yet.
    if not OUT_NPZ_DET_ALL.exists() and (era5_slopes is None or seas5_slopes is None):
        era5_slopes = build_era5_detrend_slopes(era5_clim)
        seas5_slopes = build_seas5_detrend_slopes(seas5_clim)
